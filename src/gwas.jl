...
function vgwas(
    ...
    if isplink
        ...
        bedids = SnpArrays.makestream(famfile) do io
            CSV.read(io, DataFrame)[!, 1] # assuming first column is IDs column
        end
    else
    ...
    if !inorder
        ...
        warn("Null model data was rearranged to match geneticfile order")
    end
...
function vgwas(
    ...
    if analysistype == "singlesnp"
        ...
        testdf[!, :snp] = zeros(size(fittednullmodel.nsum, 1))
                ...
                for j in eachindex(snpmask)
                    ...
                    elseif test == :wald
                        if maf == 0 # mono-allelic
                            ...
                            copyto!(pvalsβ, 1, coeftable(vlmm).cols[4], fittednullmodel.p + 1, q)
                            copyto!(pvalsτ, coeftable(vlmm).cols[4][end-q:q])
                        end
                        if snponly
...
"""
    snptodf!(df[!, :snp], snpholder, nullmodel)
